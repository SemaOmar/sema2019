...
ltl spec1{ []( (!(state == OFF && pulsacion)) | <>((state == OFF && pulsacion) & <>(state == DIG1)) ) }
...
active proctype codigo(){
	...
	:: (state == OFF) -> atomic {
		if
		:: pulsacion -> printf("Introduccion Codigo\n"); pulsacion = 0; time_out = 0; state = DIG1;
		:: skip;
		fi
	}
	:: (state == DIG1) -> atomic {
		...
		:: pulsacion && !time_out -> printf("Digito incrementado\n"); pulsacion = 0;
		:: skip;
		fi
	}
	:: (state == DIG2) -> atomic {
